...
): keyof typeof PROFILE_GROUPS | null => {
  for (const [group, profiles] of Object.entries(PROFILE_GROUPS) as Array<
    [keyof typeof PROFILE_GROUPS, string[]]
  >) {
...
const buildMatrix = async (
  ...
  if (cellCount > maxCells) {
    matrixWarnings.push(
      `Matrix size ${cellCount} exceeds limit ${maxCells}. Falling back to haversine distances.`
    );
  ...
  try {
    ...
    matrixWarnings.push(
      `ORS matrix request failed (${(error as Error).message}). Using haversine fallback.`
    );
    return buildFallbackMatrix(locations, options.averageSpeedKmh ?? DEFAULT_OPTIONS.averageSpeedKmh, matrixWarnings);
  }
};
...
const buildIsochrones = async (
  ...
  const effectiveLimit =
    limits.isochroneMaxLocations ?? DEFAULT_LIMITS.isochrones.maxLocations;
  const maxRequests = Math.min(
    options.maxIsoRequests ?? DEFAULT_OPTIONS.maxIsoRequests,
    effectiveLimit
  );
  ...
  switch (geometry.type) {
    case 'FeatureCollection': {
      ...
      return collection.features.some((feature) =>
        isPointInsideFeature(feature, coordinate)
      );
    }
    case 'Feature': {
...
const bestInsertionIndex = (
  ...
  for (let insertAt = 0; insertAt <= sequence.length; insertAt += 1) {
    ...
    const nextIndex =
      insertAt === sequence.length ? 0 : sequence[insertAt]?.matrixIndex ?? 0;
    const costIncrease =
...
const buildTour = (
  ...
  while (remaining.size > 0) {
    ...
    for (const candidate of remaining.values()) {
      ...
      if (
        isochrone &&
        !isPointInsideFeature(isochrone, candidate.coordinate)
      ) {
    ...
    if (plannedStops.length >= options.maxCandidatesPerTour) {
      warnings.push(
        `Reached max candidates per tour constraint (${options.maxCandidatesPerTour}).`
      );
      break;
    }
  }
...
const enrichWithDirections = async (
  ...
  const directionsLimit =
    limits.directionsMaxWaypoints ?? DEFAULT_LIMITS.directions.maxWaypoints;
...
const injectAlongRouteClients = (
  ...
  for (const candidate of remaining.values()) {
    ...
    const result = computeRouteProximity(
      tour.routeGeoJson,
...
export const planDeliveryTours = async (
  ...
  for (const seed of seeds) {
    ...
    const routeGeometry = await enrichWithDirections(
      ors,
      mergedOptions.profile,
      depot,
      context,
  ...
  if (remaining.size > 0) {
    for (const client of remaining.values()) {
      if (client.weightKg > request.truckCapacityKg) {
        globalWarnings.push(
          `Client "${client.name}" exceeds truck capacity and remains unassigned.`
        );
      }
    }
    globalWarnings.push(
      `${remaining.size} client(s) remain unassigned after planning.`
    );
  }
