...
const findFirstPointFeature = (response?: GeocodeResponse | null): GeocodeMatch | null => {
...
const featureContainsTown = (feature: GeocodeFeature, normalizedTown: string): boolean => {
	if (!normalizedTown) {
		return false
	}
	...
	const candidateKeys = ["locality", "name", "city", "localadmin", "county", "region", "state", "label"]
...
const pickAutocompleteMatch = (response: GeocodeResponse | undefined, town: string): GeocodeMatch | null => {
...
export const geocodeTownZipLookup = async (
	...
	const structuredSize =
		typeof query.structuredSize === "number" && query.structuredSize > 0 ? query.structuredSize : undefined
	const autocompleteSize =
		typeof query.autocompleteSize === "number" && query.autocompleteSize > 0 ? query.autocompleteSize : undefined

	if (!zip && !town) {
		throw new Error('geocodeTownZipLookup requires at least a "zip" or "town" input.')
	}

	const buildStructuredParams = (overrides: Record<string, unknown>): GeocodeCommonParams => {
