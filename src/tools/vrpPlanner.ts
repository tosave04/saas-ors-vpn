...
const normalizeClient = (
  ...
  options: Pick<
    VRPPlannerOptions,
    'priorityBase' | 'priorityAgeScale' | 'urgentPriorityBoost'
  >
): NormalizedClient => {
...
const toServiceSeconds = (serviceMinutes?: number): number => {
  const minutes = Number.isFinite(serviceMinutes)
    ? (serviceMinutes as number)
...
const metersToKm = (value?: number): number =>
  typeof value === 'number' ? value / 1000 : 0;

const secondsToMinutes = (value?: number): number =>
  typeof value === 'number' ? value / SECONDS_PER_MINUTE : 0;
...
export const planDeliveryToursVRP = async (
  ...
  const shiftDurationSeconds = Math.max(
    SECONDS_PER_HOUR,
    Math.round(shiftDurationHours * SECONDS_PER_HOUR)
  );
  ...
  const clientByJobId = new Map<number, NormalizedClient>(
    normalizedClients.map((client) => [client.jobId, client])
  );
  ...
  try {
    response = await ors.optimization<OptimizationResponseLike>(
      optimizationRequest,
      options.optimizationRequestOptions
    );
  } catch (error) {
    throw new Error(
      `ORS optimization failed: ${(error as Error).message ?? 'Unknown error'}`
    );
  }
  ...
  for (const client of normalizedClients) {
    if (!assignedJobIds.has(client.jobId) && !unassignedClients.includes(client)) {
      unassignedClients.push(client);
      warnings.push(
        `Client "${client.name ?? client.id}" has no assigned route in the solver response.`
      );
    }
  }
