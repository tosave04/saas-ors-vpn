...
const cloneLimits = (limits: ORSLimits): ORSLimits => JSON.parse(JSON.stringify(limits))
...
const getProfileGroup = (profile: DirectionsProfile): DirectionModeGroup => {
	for (const [group, profiles] of Object.entries(PROFILE_GROUPS) as Array<[DirectionModeGroup, string[]]>) {
...
const appendCoordinates = (geometry: GeoJsonObject, sink: Array<[number, number]>): void => {
...
const flattenCoordinates = (feature: Feature | FeatureCollection | GeoJsonObject): Array<[number, number]> => {
...
export class ORS {
	 ...
	 * The constructor configures custom base URLs, hydrates defaults for downstream calls, and reads the API key from options or environment variables.
	...
	constructor(options: ORSClientOptions = {}) {
		...
		const resolvedApiKey = options.apiKey ?? process.env[DEFAULT_ENV_KEY]
		if (!resolvedApiKey) {
			throw new Error(
	 ...
	 * ```
	 */
	private async runRequest<T>(config: AxiosRequestConfig, options?: RequestOptions): Promise<T> {
	 ...
	 * ```
	 */
	private validateDirections(profile: DirectionsProfile, request: DirectionsRequest): void {
		...
		if (request.coordinates.length > this.limits.directions.maxWaypoints) {
			throw new Error(`Directions request exceeds ${this.limits.directions.maxWaypoints} waypoints limit.`)
		}
		...
		if (hasRestrictions) {
			...
			const restrictionLimit = this.limits.directions.maxRestrictedDistanceKmByGroup[group]
			if (restrictionLimit !== undefined && straightDistance > restrictionLimit) {
				throw new Error(
		...
		if (roundTrip?.length !== undefined) {
			...
			if (roundTripLengthKm > this.limits.directions.maxRoundTripDistanceKm) {
				throw new Error(`Round trip length exceeds ${this.limits.directions.maxRoundTripDistanceKm} km limit.`)
			}
		}

		if (options.alternative_routes?.target_count !== undefined) {
			if (options.alternative_routes.target_count > this.limits.directions.maxAlternativeRoutes) {
				throw new Error(`Alternative routes target_count cannot exceed ${this.limits.directions.maxAlternativeRoutes}.`)
			}
		}

		const avoidGeometry = options.avoid_polygons ?? options.avoid_areas
		if (avoidGeometry && isGeoJsonObject(avoidGeometry)) {
			...
			if (areaKm2 > this.limits.directions.maxAvoidPolygonAreaKm2) {
				throw new Error(`Avoid areas exceed ${this.limits.directions.maxAvoidPolygonAreaKm2} km² limit.`)
			}

			const bbox = computeGeometryBbox(avoidGeometry)
			if (bbox) {
				const width = haversineDistanceKm([bbox[0], bbox[1]], [bbox[2], bbox[1]])
				const height = haversineDistanceKm([bbox[0], bbox[1]], [bbox[0], bbox[3]])
				if (Math.max(width, height) > this.limits.directions.maxAvoidPolygonExtentKm) {
					throw new Error(`Avoid polygon extent exceeds ${this.limits.directions.maxAvoidPolygonExtentKm} km limit.`)
				}
			}
		}
	}
	 ...
	 * ```
	 */
	private validateIsochrones(profile: IsochroneProfile, request: IsochroneRequest): void {
		...
		if (request.locations.length > this.limits.isochrones.maxLocations) {
			throw new Error(`Isochrones request exceeds ${this.limits.isochrones.maxLocations} locations limit.`)
		}
		...
		if (request.range.length > this.limits.isochrones.maxIntervals) {
			throw new Error(`Isochrones request exceeds ${this.limits.isochrones.maxIntervals} range intervals limit.`)
		}
		...
		for (const value of request.range) {
			if (rangeType === "distance") {
				const valueKm = request.units === "km" ? value : metersToKilometers(value)
				if (valueKm > this.limits.isochrones.maxRangeDistanceKm) {
					throw new Error(`Isochrone distance range exceeds ${this.limits.isochrones.maxRangeDistanceKm} km limit.`)
				}
			} else {
				...
				if (maxHours !== undefined && valueHours > maxHours) {
					throw new Error(`Isochrone time range exceeds ${maxHours} hours for ${group} profiles.`)
				}
			}
		}
	}
	...
	private validateMatrix(request: MatrixRequest): void {
		...
		if (
			(typeof request.sources === "string" || typeof request.destinations === "string") &&
	...
	private validateSnap(request: SnapRequest): void {
		...
		if (request.locations.length > this.limits.snap.maxLocations) {
			throw new Error(`Snap request exceeds ${this.limits.snap.maxLocations} coordinate limit.`)
		}
	}
	...
	private validatePois(request: PoisRequest): void {
		...
		if (request.request === "bbox" && request.geometry.bbox) {
			...
			if (area > this.limits.pois.maxBboxAreaKm2) {
				throw new Error(`POIs bbox area exceeds ${this.limits.pois.maxBboxAreaKm2} km² limit.`)
			}
		}
		if (request.request === "radius" && request.geometry.radius !== undefined) {
			...
			if (radiusKm > this.limits.pois.maxSearchRadiusKm) {
				throw new Error(`POIs radius exceeds ${this.limits.pois.maxSearchRadiusKm} km limit.`)
			}
		}
		if (request.geometry.geojson && isGeoJsonObject(request.geometry.geojson)) {
			const areaKm2 = featureCollectionAreaKm2(request.geometry.geojson)
			if (areaKm2 > this.limits.pois.maxBboxAreaKm2) {
				throw new Error(`POIs polygon area exceeds ${this.limits.pois.maxBboxAreaKm2} km² limit.`)
			}
			...
			if (bbox) {
				...
				if (width * height > this.limits.pois.maxBboxAreaKm2) {
					throw new Error(`POIs search area exceeds ${this.limits.pois.maxBboxAreaKm2} km² limit.`)
				}
			}

			const geojsonValue = request.geometry.geojson
			const geometry = (geojsonValue as Feature)?.geometry ?? geojsonValue
			if (geometry?.type === "LineString") {
				const length = linestringLengthKm(geometry as LineString)
				if (length > this.limits.pois.maxLinestringLengthKm) {
					throw new Error(`POIs linestring length exceeds ${this.limits.pois.maxLinestringLengthKm} km limit.`)
				}
			}
		}
	}
	...
	private validateElevationLine(request: ElevationLineRequest): void {
		...
		if (coordinates && coordinates.length > this.limits.elevation.maxVertices) {
			throw new Error(`Elevation line requests cannot exceed ${this.limits.elevation.maxVertices} vertices.`)
		}
	}
	...
	private validateOptimization(request: OptimizationRequest): void {
		...
		if (request.jobs.length > this.limits.optimization.maxRoutes) {
			throw new Error(`Optimization request exceeds ${this.limits.optimization.maxRoutes} jobs limit.`)
		}
		...
		if (request.vehicles.length > this.limits.optimization.maxVehicles) {
			throw new Error(`Optimization request exceeds ${this.limits.optimization.maxVehicles} vehicles limit.`)
		}
	}
	...
	async directions<T = unknown>(
		...
		const acceptHeader =
			format === "gpx" ? "application/gpx+xml" : format === "geojson" ? "application/geo+json" : "application/json"
	 ...
	 * ```
	 */
	async matrix<T = unknown>(profile: MatrixProfile, request: MatrixRequest, options?: RequestOptions): Promise<T> {
		const resolvedProfile = this.ensureProfile(profile)
	 ...
	 * ```
	 */
	async optimization<T = unknown>(request: OptimizationRequest, options?: RequestOptions): Promise<T> {
	 ...
	 * ```
	 */
	async snap<T = unknown>(profile: SnapProfile, request: SnapRequest, options?: RequestOptions): Promise<T> {
		const resolvedProfile = this.ensureProfile(profile)
	 ...
	 * ```
	 */
	async pois<T = unknown>(request: PoisRequest, options?: RequestOptions): Promise<T> {
	 ...
	 * ```
	 */
	async elevationPoint<T = unknown>(request: ElevationPointRequest, options?: RequestOptions): Promise<T> {
	 ...
	 * ```
	 */
	async elevationLine<T = unknown>(request: ElevationLineRequest, options?: RequestOptions): Promise<T> {
	 ...
	 * ```
	 */
	async geocodeSearch<T = unknown>(params: GeocodeCommonParams, options?: RequestOptions): Promise<T> {
	 ...
	 * ```
	 */
	async geocodeReverse<T = unknown>(params: GeocodeCommonParams, options?: RequestOptions): Promise<T> {
		if (
			...
			throw new Error('Geocode reverse requires "point.lat" and "point.lng" numeric parameters.')
		}
	 ...
	 * ```
	 */
	async geocodeAutocomplete<T = unknown>(params: GeocodeCommonParams, options?: RequestOptions): Promise<T> {
		if (typeof params.text !== "string" || params.text.trim().length === 0) {
			throw new Error('Geocode autocomplete requires a non-empty "text" parameter.')
		}
	 ...
	 * ```
	 */
	async geocodeStructured<T = unknown>(params: GeocodeCommonParams, options?: RequestOptions): Promise<T> {
